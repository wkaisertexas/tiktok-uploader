...
def get_uploader_args():
	...
	# selenium arguments
	parser.add_argument('--headless', action='store_true', default=False, help='Runs the program in headless mode (no browser window)')
...
def auth():
	...
	cookies = login_accounts(accounts=login_info)

	for username, cookie in cookies.items():
		save_cookies(cookie, username, join(args.output, username + '.txt'))
...
def get_auth_args():
	...
	# authentication arguments
	parser.add_argument('-o', '--output', default='tmp', help='The output folder to save the cookies to')
	parser.add_argument('-i', '--input', help='The input file which contains the username and password in a csv format')
	# parser.add_argument('-h', '--header', default=True, help='The header of the csv file which contains the username and password')
	parser.add_argument('-u', '--username', help='Your TikTok email / username')
	parser.add_argument('-p', '--password', help='Your TikTok password')
